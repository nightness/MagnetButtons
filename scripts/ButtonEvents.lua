...
-- The secure template uses clicks, so the addon uses OnMouseDown/OnMouseUp to watch for clicks
function addon.OnMouseDown(self, button)
	local shiftDown = IsShiftKeyDown();
	local ctrlDown  = IsControlKeyDown();
	local altDown   = IsAltKeyDown();
	local noCombat = (not UnitAffectingCombat("player")) and (not InCombatLockdown());
	...
	elseif ((not self.IsMoving) and (button == "LeftButton") and ctrlDown and noCombat) then
		addon.ShowProps(self);
	else
		addon.ProcessTextureChange(self, button)
	end
end
...
-- Process the texture change based on the button clicked
function addon.ProcessTextureChange(self, button)
	DoIt.Debug("ToDo: Process texture changed based on the button clicked... "..tostring(button))
end

-- A button just received a drop from the cursor
function addon.OnReceiveDrag(self)
	...
	DoIt.Debug("OnReceiveDrag: "..tostring(infoType)..", "..tostring(info1)..", "..tostring(info2)..", "..tostring(info3)..", "..tostring(inLockdown));
	...
	elseif (infoType == "spell") then
		...
		--  spell = spellName
		--})
		--DoIt.Debug(format("OnReceiveDrag: SetSpellBookItem: "..tostring(spellName)));
	...
	elseif (infoType == "mount") then
		...
		--DoIt.Debug(format("Type_Mount: "..tostring(mountName)..", "..tostring(mountIndex)..", textureId = "..tostring(textureId)));
	...
	elseif (infoType == "macro") then
		...
		--DoIt.Debug(format("Type_Macro: "..tostring(mountName)..", "..tostring(mountIndex)..", textureId = "..tostring(textureId)));
	...
	elseif (infoType == "flyout") then
		--DoIt.Debug("Flyout drop detected... "..tostring(info1)..", "..tostring(info2));
...
function addon.OnEvent(self, event, arg1, arg2, ...)
	...
	if (event == "RUNE_POWER_UPDATE") and (arg1 ~= "player") then
		-- DoIt.Debug("UNIT_RUNIC_POWER, "..tostring(arg1)..", "..tostring(arg2));
		return;
	end
	if (event == "ACTIONBAR_UPDATE_STATE") then
		--DoIt.Debug("ACTIONBAR_UPDATE_STATE: "..tostring(arg1)..", "..tostring(arg2));
	end
	if (event == "ACTIONBAR_UPDATE_USABLE") then
		---DoIt.Debug("ACTIONBAR_UPDATE_USABLE: "..tostring(arg1)..", "..tostring(arg2));
	end
	if (event == "ACTIONBAR_UPDATE_COOLDOWN") then
		---DoIt.Debug("ACTIONBAR_UPDATE_COOLDOWN: "..arg1..", "..arg2);
	end
	...
	elseif (event == "UNIT_SPELLCAST_SUCCEEDED") then
		-- DoIt.Debug("UNIT_SPELLCAST_SUCCEEDED: "..tostring(arg2));
		if (arg1 == "player") and (self:GetAttribute("type") == "spell") and (self:GetAttribute("spell") == arg2) then
			-- DoIt.Debug("UNIT_SPELLCAST_SUCCEEDED: HOOKED");
		end
	elseif (event == "COMBAT_LOG_EVENT") and (arg2 == "SPELL_AURA_APPLIED") and (arg4 == UnitName("player")) and (arg10 == self:GetAttribute("spell") and (self:GetAttribute("type") == "spell"))  then
		-- DoIt.Debug("SPELL_AURA_APPLIED: "..tostring(arg10));
	...
	elseif (event == "COMBAT_LOG_EVENT") and (arg2 == "SPELL_AURA_REMOVED") and (arg4 == UnitName("player")) and (arg10 == self:GetAttribute("spell") and (self:GetAttribute("type") == "spell")) then
		-- DoIt.Debug("SPELL_AURA_REMOVED: "..tostring(arg10));
	...
	elseif (event == "COMPANION_UPDATE") and (arg1 ~= "MOUNT") then
		...
		-- DoIt.Debug("COMPANION_UPDATE: "..tostring(arg1)..", "..tostring(issummoned));
